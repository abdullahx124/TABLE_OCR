...
def add_serial_number_with_header(data, starting = 0):
    header_row = data[0]
    ...
    for index, row_data in enumerate(data[1:], start=1):
        row_data.insert(0, f"{index + starting}")
        data_with_serial.append(row_data)

    return data_with_serial


def Collective_function(pdf_name, condition):
    pdf_path = pdf_name
    ...
    for x, y in zip(cell_coordinates, cropped_table_list):
        ...
        if count == 0:
            ...
            count = len(data_with_serial_and_header) - 1
        else:
            data_with_serial_and_header = add_serial_number_with_header(data_1, starting=count)
            count = len(data_with_serial_and_header) - 1
        import csv
